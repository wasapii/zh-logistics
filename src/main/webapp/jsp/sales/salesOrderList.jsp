...
<script type="text/javascript" src="<%=basePath %>js/salesOrderList.js"></script>
...
<body>
		...
		<table style="width:100% ;border: 0;">
			<tr>
				...
				<td width="100%" height="29" valign="top"
					...
					<div class="titlebt">销售出库单列表</div></td>
	...
	<div id="context" style="background-color:#EEF2FB;height: 100%">
		...
		<form action="salesOrderAction!querySalesOrder.action?page.localPage=1"
			method="post">
			<table>
				<tr>
					<td>日期 ：</td>
		...
		<button  id = "updatesalesOrderAction" name = "updateSalesOrderAction">修改</button>
		<button  id = "delete" name = "delete">删除</button>
		<a href="<%=basePath %>jsp/sales/addSalesOrder.jsp">新增</a>
		...
		<div align="right" style="width:1600px;height: 50px">
			...
			<s:else>
				<a href="salesOrderAction!querySalesOrder.action?page.localPage=1&invoice.startDate=${invoice.startDate}&invoice.endDate=${invoice.endDate}">首页</a>&nbsp
		  		<s:if test="page.localPage < page.totalPage&&page.localPage == 1">
		  			上一页&nbsp<a href="salesOrderAction!querySalesOrder.action?page.localPage=${page.localPage + 1}&invoice.startDate=${invoice.startDate}&invoice.endDate=${invoice.endDate}">下一页</a>&nbsp
		    	</s:if>
				<s:elseif test="page.localPage == page.totalPage">
					<a href="salesOrderAction!querySalesOrder.action?page.localPage=${page.localPage - 1}&invoice.startDate=${invoice.startDate}&invoice.endDate=${invoice.endDate}">上一页</a>&nbsp下一页&nbsp
		    	</s:elseif>
		    	<s:else>
		    		<a href="salesOrderAction!querySalesOrder.action?page.localPage=${page.localPage - 1}&invoice.startDate=${invoice.startDate}&invoice.endDate=${invoice.endDate}">上一页</a>&nbsp
		    		<a href="salesOrderAction!querySalesOrder.action?page.localPage=${page.localPage + 1}&invoice.startDate=${invoice.startDate}&invoice.endDate=${invoice.endDate}">下一页</a>&nbsp
		    	</s:else>
				<a href="salesOrderAction!querySalesOrder.action?page.localPage=${page.totalPage}&invoice.startDate=${invoice.startDate}&invoice.endDate=${invoice.endDate}">最末页</a>
			</s:else>
