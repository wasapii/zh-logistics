function showDetail(note){
	var id = note.id;
	$.post("salesOrderJson!queryDetail.action",{id:id},function(data){
...
$(document).ready(function(){
	$("#delete").click(function() {
		...
		if (length == 0) {
			...
			$.post("salesOrderAction!deleteSalesOrder.action",{id:id},function(){
				location.href = "salesOrderAction!querySalesOrderAll.action";
			});
		}
	});
	//修改
	$("#updatesalesOrder").click(function(){
		...
		if(checkboxLength==0){
			...
			$.post("",function(){
				location.href = "salesOrderAction!toUpdateSalesOrder.action?invoice.id="+checkNum;
			}); 
		}
	});
});
